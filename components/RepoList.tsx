...
import type { GitHubRepo, DeepSeekConfig } from "@/lib/types";
import { Star, GitFork, ExternalLink, Clock, Search } from "lucide-react";
...
export default function RepoList({ repositories, config }: RepoListProps) {
  const [analyzingRepo, setAnalyzingRepo] = useState<GitHubRepo | null>(null);
          ...
          {topRepos.map((repo) => (
            ...
            >
              <div className="flex items-start justify-between gap-4">
                ...
                {/* 深度分析按钮 */}
                <button
                  onClick={() => setAnalyzingRepo(repo)}
                  className="flex-shrink-0 px-2.5 py-1.5 bg-purple-600/10 text-purple-300 border border-purple-500/20 rounded-lg text-xs hover:bg-purple-600/20 transition-colors flex items-center gap-1"
                  title="AI 深度分析此仓库"
                >
